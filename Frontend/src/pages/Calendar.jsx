...
import { Calendar, Info, Share } from 'lucide-react';
...
const CalendarPage = () => {
  ...
  const startDate = useMemo(() => {
    ...
    return new Date(strategy.startDate || strategy.createdAt);
  }, [strategy]);

  const plan = useMemo(() => strategy?.generatedPlan || null, [strategy]);

  const headerText = useMemo(() => {
    if (!startDate) return "Loading plan...";
    if (strategy.endDate) {
      const endDate = new Date(strategy.endDate);
      return `A custom plan from ${startDate.toLocaleDateString()} to ${endDate.toLocaleDateString()}.`;
    }
    return `A 30-day plan starting from ${startDate.toLocaleDateString()}.`;
  }, [startDate, strategy]);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="min-h-screen bg-gray-900 text-white p-6">
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="flex justify-between items-center mb-8 flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-2">{headerText}</p>
          </div>
          <button onClick={handleExportCSV} className="flex items-center px-4 py-2 bg-gradient-to-r from-green-500 to-blue-600 rounded-xl text-white font-semibold hover:from-green-600 hover:to-blue-700 transition-all duration-300">
            ...
            <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-3xl p-6 shadow-xl border border-gray-700">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex space-x-2">
                  <select value={selectedMonth} onChange={(e) => setSelectedMonth(parseInt(e.target.value))} className="bg-gray-700 text-white rounded-lg px-3 py-1 text-sm">
                    {monthNames.map((month, index) => (<option key={index} value={index}>{month}</option>))}
                  </select>
                  <select value={selectedYear} onChange={(e) => setSelectedYear(parseInt(e.target.value))} className="bg-gray-700 text-white rounded-lg px-3 py-1 text-sm">
                    {[...Array(3)].map((_, i) => (<option key={i} value={new Date().getFullYear() - 1 + i}>{new Date().getFullYear() - 1 + i}</option>))}
                  </select>
                </div>
              </div>

              <div className="grid grid-cols-7 gap-1 mb-4">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (<div key={day} className="text-center text-gray-400 text-sm font-semibold py-2">{day}</div>))}
              </div>

              <div className="grid grid-cols-7 gap-1">
                {calendarDays.map((day, index) => (
                  <motion.div key={index} whileHover={{ scale: 1.05 }} className={`aspect-square flex items-center justify-center text-sm font-medium rounded-lg cursor-pointer transition-all duration-200 relative group ${day.isCurrentMonth ? 'text-white' : 'text-gray-500'} ${day.isToday ? 'bg-blue-500 text-white' : ''} ${day.hasContent ? 'bg-green-500 text-white' : 'hover:bg-gray-700'}`}>
                    {day.day}
                    {day.hasContent && (
              ...
              <div className="mt-6 flex items-center justify-center space-x-6 text-xs">
                <div className="flex items-center"><div className="w-3 h-3 bg-blue-500 rounded-full mr-2"></div><span className="text-gray-400">Today</span></div>
                <div className="flex items-center"><div className="w-3 h-3 bg-green-500 rounded-full mr-2"></div><span className="text-gray-400">Scheduled</span></div>
              </div>
            </div>
          </motion.div>
        </div>
